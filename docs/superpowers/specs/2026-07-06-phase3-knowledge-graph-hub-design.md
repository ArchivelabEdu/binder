...
| D7 | **API 완결성 + XAI 횡단 원칙 + 후속 트랙 4종**: 모든 관리 기능은 REST API(상태 변경은 bearer 인증 — CSRF 불변식 유지)로도 수행 가능해야 하고(향후 바이브 코딩 운용의 전제), AI가 개입한 모든 변이는 모델·버전·프롬프트·검수자까지 추적 가능해야 한다(XAI, §4). 워크플로우 자동화·스킬/MCP·AI 오퍼레이터는 후속 트랙(§10)으로 정의 (사용자 지시 2026-07-06) | 현대차 12WF/55Job 워크플로우 모델 + 아카이브시스템101의 워크플로우 엔진 권고(n8n+AI Agent, "보존 업무는 인적 개입 없이 지속"); 사전질문지 Q2(XAI·PROV-O·AI 생성 표기)·Q3(AI 파이프라인의 인간 개입 지점 설계) |
...
└──────────────────────────────────────────────────────────────────────────┘
        ↓ 발행(JSON-LD·변경 피드)                  ┈┈> Omeka S 접근 사본 발행 (후속 트랙 §10)
...
- **API 완결성 원칙(D7)**: Binder의 모든 관리 기능은 UI 없이 REST API로도 수행 가능해야 한다(상태 변경은 bearer 인증 — CSRF 불변식 유지). 후속 트랙의 워크플로우 엔진(§10 T2)과 AI 오퍼레이터(§10 T4)가 이 API 표면을 도구로 소비한다. P3.1부터 신규 기능은 API 우선으로 설계하고, 기존 Server Action 전용 기능의 API 병행 노출은 후속 트랙에서 일괄 정리한다.
...
- **내부 모델은 하나**(Record/RecordSet/Instantiation/Agent/Event/Concept + 관계 1급 객체, RiC-O 정렬)이고, **외부 어휘 표현은 여럿**이다. 이를 위해:
  - `vocabularies`: 등록된 외부 어휘(prefix, 네임스페이스 URI, 설명). 시드: rico, la(Linked Art), skos, schema, foaf, dcterms, prov(PROV-O — XAI 계보 표현 대비, D7) — 관리자 등록으로 확장(CIDOC CRM 등).
...
- **변경 피드**: 시간순 변경 스트림 API. 사양을 명시한다:
  ...
  - **행위 주체 컨텍스트(D7, P3.1로 당김)**: audit_log에 행위 주체 구분(사람 사용자 | AI 파이프라인 | 워크플로우)과 주체 참조를 확장 — XAI 추적(§4)과 후속 트랙(§10)의 공통 기반.
...
- **XAI 기록(D7)**: extract_jobs에 모델 ID·버전·프로바이더·프롬프트(스냅샷 또는 해시+원문 보관)·파라미터를 기록한다. 커밋된 관계는 extract_candidates → extract_jobs로 역추적 가능해야 한다 — **입력 → 모델(버전) → 트리플 후보 → 검수자 판단 → 커밋의 전 계보**. PROV-O 매핑은 어휘 레지스트리의 prov 등록(§2.7)으로 대비하고, AttributeAssignment식 정교화(사전질문지 Q2)는 후속.
...
- Binder→Omeka S 접근 사본 발행 + 파생본(ffmpeg) 파이프라인 — 후속 트랙 T1(§10)
...
- 워크플로우 엔진, 스킬/MCP 계층, AI 오퍼레이터 — "제외"가 아니라 **후속 트랙(§10)으로 이연**. P3.1~P3.4가 그 선행 훅(API 완결성·변경 피드·행위 주체 감사·XAI 기록)을 심는다

## 10. 후속 트랙 (Phase 3+, D7 — 순서는 P3.4 완료 시 협의)

P3.1~P3.4 코어 위에 얹는 오케스트레이션·AI 운용 계층. 여기서 미리 정의하되 구현 플랜은 각 트랙 착수 시 JIT 작성한다.

### T1. 접근 계층 발행

Binder→Omeka S 접근 사본 발행 + 파생본(ffmpeg) 파이프라인. Omeka S는 큐레이션된 전시·발행 프런트(§1)로, Binder가 메타데이터(JSON-LD/매핑 프로파일)와 접근 사본을 공급한다.

### T2. 워크플로우 자동화

현대차 완료보고서의 12 WF/55 Job 모델(트리거 = 일자 도래·이벤트 → 검토 → 조치계획 → 승인 → 실행 → 로그)과 아카이브시스템101의 권고(유연한 워크플로우 엔진 + 자동화 트리거, n8n+AI Agent 예시)를 채택하되, **자체 엔진을 만들지 않고 n8n류 엔진을 통합**한다:

- **Binder가 제공**: (a) 트리거 소스 — 변경 피드(§3.4) + 웹훅 푸시 + 일자 도래 스캐너(공개 재분류 예정일·재평가 예정일 등 메타데이터 일자 필드), (b) 액션 API — bearer 인증 REST(D7 API 완결성), (c) **인간 승인 게이트** — 검수 큐 패턴(§4)의 일반화(워크플로우가 승인 요청을 생성하고 승인 후 재개).
- **엔진이 담당**: 워크플로우 정의·실행·재시도·모니터링, Archivematica·Omeka S·AtoM API 호출 노드로 시스템 간 업무 자동화, **커뮤니티 공유 워크플로우**(n8n 템플릿 생태계) 수입.
- 조직의 아카이브 업무(입수 검수, 정기 무결성, 공개 재분류, 처분 등)를 워크플로우로 정의하고 각 단계에 적절한 도구(시스템 API·AI·사람)를 배치한다.

### T3. 스킬·MCP 계층

- **발행**: 검증·성숙한 워크플로우를 **스킬 패키지**(워크플로우 정의 + 실행 문서 + 필요 도구 목록 + 버전)로 저장·관리하고, **Binder MCP 서버**(엔터티 조회·그래프 질의·액션 도구)와 REST API를 통해 조직 내외부에 제공한다.
- **소비**: 외부 스킬·MCP 도구·SaaS를 T2 워크플로우와 T4 AI 오퍼레이터에서 도구로 가져와 사용한다(n8n의 MCP/AI Agent 노드 또는 에이전트 클라이언트의 MCP 연결).

### T4. 바이브 코딩 운용 (AI 오퍼레이터)

Archivematica·Omeka S·AtoM·Binder를 각 시스템의 GUI가 아니라 **REST API를 도구로 쥔 대화형 AI(채팅·음성)** 로 관리·운용한다:

- 구조 = 백엔드별 MCP 서버(각 시스템 API 래핑) + Binder MCP(신원·그래프의 허브 — 어느 시스템의 어느 레코드가 같은 것인지 아는 곳) + LLM 에이전트 클라이언트.
- 파괴적·되돌리기 어려운 작업은 **인간 승인 게이트 필수**(전 구간 인간 게이트 원칙 유지). 모든 AI 조작은 행위 주체 감사(§3.4)와 XAI 기록(§4)으로 추적된다.
- 전제 = D7 API 완결성. Omeka S 파일럿에서 이미 검증된 headless 운용(REST 시딩, omeka-s-cli 키 발급)이 이 방향의 실증이다.

### 선행 훅 (P3.1~P3.4에 반영됨)

① API 완결성 원칙(§1), ② 변경 피드·웹훅의 원형(§3.4), ③ 감사 행위 주체 컨텍스트(§3.4 전제), ④ XAI 기록(§4), ⑤ 어휘 레지스트리의 prov 등록(§2.7).
